...
export const getHomeLayout = async (req, reply) => {
    try {
        ...
        const hydratedComponents = await Promise.all(components.map(async (comp) => {
            ...
            } else if (["PRODUCT_GRID", "PRODUCT_SCROLLER", "CATEGORY_CLUSTERS", "STORY_STRIP", "GRADIENT_HERO", "RAMZAN_SPECIAL", "RAMZAN_SPECIAL2", "HAPPY_HOLI", "DIWALI_SPECIAL"].includes(comp.type)) {
